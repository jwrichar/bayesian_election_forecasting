...
plot(probs_old[is_swing_state],N_swing_state:1, pch=20, axes=FALSE, ylab="",
     ...
     xlim=c(0,1), ylim=c(0.5, N_swing_state+1), col=colvec,
     main=paste("Change in Swing State Probabilities from", dates[1], "to", dates[2]))
rect(0, 0, 0.2, N_swing_state+2, border=NA, col = rgb(1,0,0,0.2))
rect(0.2, 0, 0.4, N_swing_state+2, border=NA, col = rgb(1,0,0,0.1))
rect(0.6, 0, 0.8, N_swing_state+2, border=NA, col = rgb(0,0,1,0.1))
rect(0.8, 0, 1, N_swing_state+2, border=NA, col = rgb(0,0,1,0.2))
text(0.1, N_swing_state+1, 'Likely R', cex=0.7)
text(0.3, N_swing_state+1, 'Leans R', cex=0.7)
text(0.5, N_swing_state+1, 'Toss Up', cex=0.7)
text(0.7, N_swing_state+1, 'Leans D', cex=0.7)
text(0.9, N_swing_state+1, 'Likely D', cex=0.7)
...
abline(v=0.5,col=4,lty=2,lwd=1)
dev.off()
